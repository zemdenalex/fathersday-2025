 ...
 */
async function generateVariants(originalPath, photoId) {
    const db = getDb();
    try {
...
export async function processPhoto(originalPath) {
    ...
    `).run(metadata.filename, originalPath, metadata.width, metadata.height, metadata.fileSize, metadata.mimeType, metadata.avgColor.r, metadata.avgColor.g, metadata.avgColor.b, metadata.luma, metadata.saturation);
 ...
 */
export async function reindexPhotos() {
    const db = getDb();
    let count = 0;
    try {
 ...
 */
export async function handleRebuildAtlas(req, res) {
    try {
        await buildAtlas();
 ...
 */
export async function handleReindexPhotos(req, res) {
    try {
 ...
 * Get atlas data
 */
export function getAtlas(req, res) {
    const db = getDb();
    try {
